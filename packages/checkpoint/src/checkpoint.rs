...
pub const BATCHES: Deque<Batch> = Deque::new("batches");
...
pub const PENDING: Map<Dest, Coin> = Map::new("pending");
...
impl Checkpoint {
    ...
    fn sign(
        ...
        for i in 0..BATCHES.len(store)? {
            ...
            for j in 0..batch.len() {
                ...
                for k in 0..tx.input.len() {
                    ...
                    input.signatures.sign(store, pubkey.into(), sig)?;
        ...
        if self.signed(store)? && !cp_was_signed {
    ...
    pub fn checkpoint_tx(
        ...
        Ok(Adapter::new(
            BATCHES
                .get(store, BatchType::Checkpoint as u32)?
                .unwrap()
                .last()
                .unwrap()
                .to_bitcoin_tx(store)?,
    ...
    /// status.
    pub fn reserve_output(&self, store: &dyn Storage) -> ContractResult<Option<TxOut>> {
        // TODO: should return None for Building checkpoints? otherwise this
        // might return a withdrawal
        let checkpoint_tx = self.checkpoint_tx(store)?;
    ...
    fn signed_batches(&self, store: &dyn Storage) -> ContractResult<u64> {
    ...
    /// signed.
    pub fn current_batch(&self, store: &dyn Storage) -> ContractResult<Option<Batch>> {
        if self.signed(store)? {
            return Ok(None);
        }
        let pos = self.signed_batches(store)?;
        Ok(BATCHES.get(store, pos as u32)?)
    }
    ...
    /// otherwise returns `false`.
    pub fn signed(&self, store: &dyn Storage) -> ContractResult<bool> {
        Ok(self.signed_batches(store)? == BATCHES.len(store)? as u64)
    }
    ...
    pub fn emergency_disbursal_txs(
        ...
        let intermediate_tx_batch = BATCHES
            .get(store, BatchType::IntermediateTx as u32)?
            .unwrap();
        let Some(intermediate_tx) = intermediate_tx_batch.get(0) else {
            return Ok(txs);
        };
        txs.push(Adapter::new(intermediate_tx.to_bitcoin_tx(store)?));

        let disbursal_batch = BATCHES.get(store, BatchType::Disbursal as u32)?.unwrap();
        for tx in disbursal_batch.iter() {
            txs.push(Adapter::new(tx.to_bitcoin_tx(store)?));
        }

        Ok(txs)
    }

    pub fn checkpoint_tx_miner_fees(&self, store: &dyn Storage) -> ContractResult<u64> {
        let mut fees = 0;
    ...
    pub fn base_fee(
        ...
        let est_vsize = self.est_vsize(store, config, timestamping_commitment)?;
    ...
    fn est_vsize(
        ...
        let mut tx = cp.to_bitcoin_tx(store)?;
...
//         let intermediate_tx_batch = BATCHES.get(BatchType::IntermediateTx as u64)?.unwrap();
